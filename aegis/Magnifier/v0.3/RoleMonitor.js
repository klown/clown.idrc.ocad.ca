...
(function() {
	...
	var loadRoleListWin = function() {
		...
		roleListWin.append ((jQuery ("<h3>Magnifier</h3>").css ({textAlign: "left", marginLeft: "1em"})));
		...
		fieldSet.append ("<label for='magClone'><input id='magClone' type='checkbox' name='magClone'>Overlay magnification</label><br>");
		...
		theMagClone = jQuery ("#magClone", fieldSet);
...
//				cloneOverlay = aegis.magnify.CloneAndMagnify (prevMagStyles.jRoleEl, false);
				cloneOverlay = aegis.magnify.CloneAndMagnify (prevMagStyles, false);
	...
	aegis.atrc.magOnHover = function (prevMagStyles) {
		var jEl = prevMagStyles && prevMagStyles.jRoleEl;
		if (jEl) {
			var delayAttachMouseleave = function() {
			...
			jEl.mouseenter (function() {
				...
				setTimeout (delayAttachMouseleave, 500);
			});
