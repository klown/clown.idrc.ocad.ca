...
<html lang="en">
<head>
    <title>Code coverage report for universal/gpii/node_modules/eventLog/src/eventLog.js</title>
    <meta charset="utf-8" />
...
<div class='wrapper'>
  <div class='pad1'>
    <h1>
      <a href="../../../../../index.html">All files</a> / <a href="index.html">universal/gpii/node_modules/eventLog/src</a> eventLog.js
    </h1>
    <div class='clearfix'>
...
gpii.eventLog.logError = function (that, moduleName, errType, err, level) {
    <span class="missing-if-branch" title="else path not taken" >E</span>if (!level) {
...
<span class="cstat-no" title="statement not covered" >        data.error = {};</span>
<span class="cstat-no" title="statement not covered" >        fluid.each(Object.getOwnPropertyNames(err), <span class="fstat-no" title="function not covered" >fu</span>nction (a) {</span>
<span class="cstat-no" title="statement not covered" >            data.error[a] = err[a];</span>
        });
...
&nbsp;
    <span class="missing-if-branch" title="else path not taken" >E</span>if (!data.error.stack) {
...
fluid.failureEvent.addListener(function (args) {
    var err = Array.isArray(args) ? args.join(" ") : <span class="branch-1 cbranch-no" title="branch not covered" >err;</span>
...
gpii.eventLog.gotError = function (err, errType) {
    ...
    <span class="missing-if-branch" title="else path not taken" >E</span>if (eventLog.length &gt; 0) {
...
gpii.eventLog.checkLevel = function (level) {
    <span class="missing-if-branch" title="if path not taken" >I</span>if (typeof level === "string" &amp;&amp; <span class="branch-1 cbranch-no" title="branch not covered" >level in fluid.logLevelsSpec)</span> {
<span class="cstat-no" title="statement not covered" >        return fluid.logLevel[level];</span>
    } else {
